...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        _rowHeight = 30;
        _marginWidth = 3;
        self.colorStateNormal = kColorGrey3;
        self.colorStateSelected = kColorWhite;
        self.colorSelector = kColorGray1;
    }
    return self;
}
...
- (void)buildView
{
    ...
    CGFloat width = (self.width - self.marginWidth * (count + 1)) / count;
    for (int i = 0; i < count; i++) {
        [_selectedIndexPaths addObject:[NSIndexPath indexPathForRow:0 inSection:0]];
        
        CGFloat emptyHeight = (self.height - _rowHeight) / 2;
        
        UITableView *tvData = [[UITableView alloc] initWithFrame:CGRectMake(width * i + self.marginWidth * (i + 1), 0, width, self.height)];
        ...
        UIView *vSelector = [[UIView alloc] initWithFrame:CGRectMake(tvData.minX, emptyHeight, tvData.width, _rowHeight)];
