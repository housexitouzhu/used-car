	...
	if (numberOfLevels > 0) {
		...
		for (int i = 0; i < numberOfLevels; i++) {
			...
			if (i < [self.tableViews count]) {
                ...
                // 设置tableview背景
                UIView *tableBackView = [[UIView alloc] initWithFrame:self.bounds];
                tableBackView.backgroundColor = [UIColor whiteColor];
                CGFloat marginLeft = i > 0 ? 1.0f : 0.0f;
                
                UIView *whiltBackView = [[UIView alloc] initWithFrame:CGRectMake(marginLeft, 0, tableBackView.width - marginLeft, tableBackView.height)];
                if (i != 0)
                    whiltBackView.backgroundColor = [UIColor whiteColor];
                [tableBackView addSubview:whiltBackView];
                if (i > 0) {
                    UIView *vLine = [[UIView alloc] initWithFrame:CGRectMake(0, 0, kLinePixel, tableView.height)];
                    vLine.backgroundColor = kColorNewLine;
                    [tableBackView addSubview:vLine];
                }
                tableView.backgroundView = tableBackView;
...
{
    // 修复MJNIndexView添加到列表中不能拖动问题
    return touch.view.class != NSClassFromString(@"MJNIndexView");
}
...
- (void)dragTableView:(UIPanGestureRecognizer *)panGestureRecognizer {
	...
	switch (panGestureRecognizer.state) {
		case UIGestureRecognizerStateEnded: {
            ...
            CGFloat currentMargin = _marginLifts.count > (self.currentTableView.tag - 1)?  [[_marginLifts objectAtIndex:self.currentTableView.tag - 1] floatValue] : self.marginLeft;
            
			[UIView animateWithDuration:0.2
								  delay:0.0
								options:UIViewAnimationOptionCurveEaseInOut
							 animations:^{
								 [draggedTableView setFrame:CGRectMake(draggedTableViewDefaultXCoordinate + currentMargin, self.bounds.origin.y, self.frame.size.width, self.frame.size.height)];
								 [draggedTableView.fixedTableHeaderView setCenter:CGPointMake(draggedTableView.center.x, draggedTableView.fixedTableHeaderView.center.y)];
							 }
							 completion:^(BOOL finished) {
							 }];
...
- (NSArray *)sectionIndexTitlesForTableView:(UITableView *) tableView{
    NSArray *arrKeys = nil;
    if ([self.dataSource respondsToSelector:@selector(sectionIndexTitlesForTableView:level:)]) {
		arrKeys = [self.dataSource sectionIndexTitlesForTableView:self level:self.currentTableViewIndex];
	}
	return arrKeys;
}
...
- (NSString *)tableView:(UITableView *)tableView titleForHeaderInSection:(NSInteger)section {
    ...
    if ([self.dataSource respondsToSelector:@selector(multiTablesView:slideLevel:titleForHeaderInSection:)]) {
		titleForHeaderInSection = [self.dataSource multiTablesView:self slideLevel:tableView.tag titleForHeaderInSection:section];
	}
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([self.dataSource respondsToSelector:@selector(multiTablesView:level:heightForRowAtIndexPath:)]) {
        heightForRow = [self.dataSource multiTablesView:self level:[self indexOfTableView:tableView] heightForRowAtIndexPath:indexPath.row];
	}else{
        heightForRow = 44;
    }
	return heightForRow;
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    if ([self.delegate respondsToSelector:@selector(multiTablesView:sliderLevel:viewForHeaderInSection:)]) {
		viewForHeaderInSection = [self.delegate multiTablesView:self sliderLevel:tableView.tag viewForHeaderInSection:section];
	}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if ([self.delegate respondsToSelector:@selector(multiTablesView:lastLevel:didSelectRowAtIndexPath:)] && (([self indexOfTableView:tableView] + 1) == [self numberOfLevels])) {
		[self.delegate multiTablesView:self lastLevel:[self indexOfTableView:tableView] didSelectRowAtIndexPath:indexPath];
	}
    else if ([self.delegate respondsToSelector:@selector(multiTablesView:level:didSelectRowAtIndexPath:)]) {
    ...
    if (self.automaticPush) {
        [self pushNextTableView:tableView animation:YES];
    }
}

#pragma mark Push Level
- (void)pushNextTableView:(UITableView*)tableView animation:(BOOL)animation {
    NSInteger indexOfTableView = [self indexOfTableView:tableView];
	
    for (NSInteger i = indexOfTableView + 1; i < [self numberOfLevels]; i++) {
	...
	[UIView animateWithDuration:animation ? 0.2 : 0.0
						  delay:0.0
						options:UIViewAnimationOptionCurveEaseOut
					 animations:^{
						 for (NSInteger i = self.currentTableViewIndex; i < [self numberOfLevels]; i++) {
							 UITableView *nextTableView = [self tableViewAtIndex:i];
                             [nextTableView setContentOffset:CGPointMake(0, 0)];
					 ...
					 completion:^(BOOL finished) {
						 [UIView animateWithDuration:0.2
											 ...
											 options:UIViewAnimationOptionCurveEaseOut
										  animations:^{
											  [self.currentTableView setFrame:CGRectMake(tableView.frame.origin.x + self.nextTableViewHorizontalGap + currentMargin, self.currentTableView.frame.origin.y, self.currentTableView.frame.size.width, self.currentTableView.frame.size.height)];
