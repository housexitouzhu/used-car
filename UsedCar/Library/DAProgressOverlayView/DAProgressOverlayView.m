...
typedef enum {
    ...
    DAProgressOverlayViewStateOperationFailed = 3,
...
- (void)setUp
{
    ...
    self.overlayColor = [UIColor colorWithWhite:0 alpha:0.7];
...
//    if (_progress != -1)
    ...
    if (_progress == -1) {
        ...
        CGContextSetFillColorWithColor(context, kColorRed.CGColor);
        CGRect rect1 = CGRectMake(-innerRadius / 3 / 2, -5, innerRadius / 3 - 1, innerRadius - 0.5);
        ...
        CGContextAddLineToPoint(context, width1 - radius, 0);
        CGContextAddArc(context, width1 - radius, radius, radius, -0.5 * M_PI, 0.0, 0);
        CGContextAddLineToPoint(context, width1, height1 - radius);
        CGContextAddArc(context, width1 - radius, height1 - radius, radius, 0.0, 0.5 * M_PI, 0);
        CGContextAddLineToPoint(context, radius, height1);
        CGContextAddArc(context, radius, height1 - radius, radius, 0.5 * M_PI, M_PI, 0);
        CGContextAddLineToPoint(context, 0, radius);
        CGContextAddArc(context, radius, radius, radius, M_PI, 1.5 * M_PI, 0);
        CGContextAddArc(context, -rect1.origin.x - 0.25, -8, rect1.size.width / 2, 0, 360 * (M_PI/180), 0);
...
//        _progress = (progress < 0.) ? 0. : (progress > 1.) ? 1. : progress;
        ...
        if (progress == -1) {
            self.state = DAProgressOverlayViewStateOperationFailed;
