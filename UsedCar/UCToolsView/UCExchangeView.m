...
#import "UCTopBar.h"
#import "UCWebView.h"
#import "UCExchangeHelpView.h"
#import "APIHelper.h"


@interface UCExchangeView ()<UIWebViewDelegate>
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        // Initialization code
        [UMSAgent postEvent:tool_dressing_vin_pv page_name:NSStringFromClass(self.class)];
        [self initView];
    }
    return self;
}
...
- (void)initView{
    ...
    _vWeb = [[UCWebView alloc] initWithFrame:CGRectMake(0, _tbTop.maxY, self.width, self.height - _tbTop.maxY) withWebURL:[APIHelper getToolExchangeWebURL]];
...
- (UCTopBar *)creatTopBarView:(CGRect)frame
{
    ...
    // 标题
    [vTopBar.btnTitle setTitle:@"成交换礼" forState:UIControlStateNormal];
...
-(void)onClickRightBtn:(UIButton *)btn{
    ...
    UCExchangeHelpView *exchangeHelp = [[UCExchangeHelpView alloc] initWithFrame:vcMain.vMain.bounds];
    [[MainViewController sharedVCMain] openView:exchangeHelp animateOption:AnimateOptionMoveLeft removeOption:RemoveOptionNone];
}
