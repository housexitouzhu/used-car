...
@property (nonatomic, strong) APIHelper *apiGetActivityInfo;
...
@property (nonatomic, strong) UIScrollView *svBanner;
@property (nonatomic, strong) UIImageView *ivBanner;
@property (nonatomic, strong) UCActivityModel *mActivity;

@property (nonatomic, assign) BOOL isForSearchResult;
@end

@implementation UCCarListView

- (id)initWithFrame:(CGRect)frame{
    self = [self initWithFrame:frame isForSearchResult:NO];
...
- (id)initWithFrame:(CGRect)frame isForSearchResult:(BOOL)boolean
{
    ...
    if (self) {
        ...
        _viewStyle = UCCarListViewStyleSearch;
        _isForSearchResult = boolean;
        
        if (_isForSearchResult) {
            // 默认提示语
            _strNoData = @"没有查到相关的结果";
        }
        else{
            // 默认提示语
            _strNoData = @"暂无车辆信息\n下拉重新加载";
...
{
    _isEnableActivityZone = isEnable;
    if (_isEnableActivityZone) {

//        占位, 支持多活动时用
//        if (!_svBanner) {
//            _svBanner = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, self.width, self.width/16.0*3.0)];
//            [_svBanner setBackgroundColor:kColorClear];
//            
//            
//            //关闭按钮
//            UIImage *imgHide = [UIImage imageNamed:@"banner_hide"];
//            UIButton *btnHideBanner = [[UIButton alloc] initWithFrame:CGRectMake(_ivBanner.width - imgHide.size.width - 10, (_ivBanner.height - imgHide.size.height) / 2, imgHide.size.width, imgHide.size.height)];
//            [btnHideBanner addTarget:self action:@selector(hideBaner) forControlEvents:UIControlEventTouchUpInside];
//            [btnHideBanner setImage:imgHide forState:UIControlStateNormal];
//            [_svBanner addSubview:btnHideBanner];
//            
//        }
        
        if (!_ivBanner) {
            _ivBanner = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, self.width, self.width / 16.0 * 3.0)];
            _ivBanner.userInteractionEnabled = YES;
            UITapGestureRecognizer *tapGesture = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(handleTapActivityZone)];
            [_ivBanner addGestureRecognizer:tapGesture];
            // 关闭按钮
            UIImage *imgHide = [UIImage imageNamed:@"banner_hide"];
            UIButton *btnHideBanner = [[UIButton alloc] initWithFrame:CGRectMake(_ivBanner.width - imgHide.size.width - 10, (_ivBanner.height - imgHide.size.height) / 2, imgHide.size.width, imgHide.size.height)];
            [btnHideBanner addTarget:self action:@selector(hideBaner) forControlEvents:UIControlEventTouchUpInside];
            [btnHideBanner setImage:imgHide forState:UIControlStateNormal];
            [_ivBanner addSubview:btnHideBanner];
        }
        //        // 显示前高度设置为0
        //        _ivBanner.height = 0;
        //        _tvCarList.tableHeaderView = _ivBanner;
        [self getActivityInfo];
    } else {
        _tvCarList.tableHeaderView = nil;
    }
}
...
- (void)onPull
{
    ...
    //    else
    //        [_pullRefresh performSelector:@selector(endRefreshing) withObject:nil afterDelay:0.01];
}
...
- (void)hideBaner
{
    [UMStatistics event:c_3_7_sellcar_adclose];
    
    _isEnableActivityZone = NO;
    _tvCarList.tableHeaderView = nil;
...
- (void)showBaner:(NSString *)imgUrl
{
    [_ivBanner sd_setImageWithURL:[NSURL URLWithString:imgUrl] placeholderImage:[UIImage imageNamed:@"activities"]];
    _tvCarList.tableHeaderView = _ivBanner;
//    [UIView animateWithDuration:0.2 animations:^{
//        _ivBanner.height = self.width / 16.0 * 3.0;
//    } completion:^(BOOL finished) {
//        _tvCarList.tableHeaderView = _ivBanner;
//    }];
}
...
- (void)handleTapActivityZone
{
    if (_mActivity) {
        // 统计
        [UMStatistics event:c_3_7_sellcar_click label:((AdlistItemModel *)_mActivity.adlist[0]).title];
        
        NSString *url = ((AdlistItemModel *)_mActivity.adlist[0]).url;
        // 跳转
        UCActivityView *vActivity = [[UCActivityView alloc] initWithFrame:self.bounds withActivityModel:_mActivity.adlist[0]];
...
- (void)tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row == _mCarLists.count - 1) {
        // 满页情况下才有下一页 & 没有正在加载更多
        if (_mCarLists.count % _pageSize == 0) {
...
- (void)searchCar
{
    ...
    __weak UCCarListView *vCarList = self;
    ...
    // 搜索接口
    if (_viewStyle == UCCarListViewStyleSearch) {
        [_apiSearchCar searchCarWithKeyword:self.keyword pagesize:[NSNumber numberWithInt:_pageSize] pageindex:[NSNumber numberWithInteger:vCarList.pageIndex] areaid:self.mArea.areaid pid:self.mArea.pid cid:self.mArea.cid dealerid:self.dealerid filter:self.mFilter orderby:self.orderby];
    }
    // 根据商家id和用户手机号获取用户关注的在售、已售、已过期车辆信息
    else if (_viewStyle == UCCarListViewStyleAttention)
        [_apiSearchCar getUserAttentionCarListWithMobile:_mobile state:[NSNumber numberWithInteger:_state]];
    else if (_viewStyle == UCCarListViewStyleShareCarList)
        [_apiSearchCar getShareCarListWithShareID:[NSNumber numberWithInteger:_shareID] page:vCarList.pageIndex size:_pageSize];
    
}

- (void)getActivityInfo
{
    if (!_apiGetActivityInfo)
        _apiGetActivityInfo = [[APIHelper alloc] init];
    else
        [_apiGetActivityInfo cancel];
    
    __weak UCCarListView *vCarList = self;
    
    
    // 设置请求完成后回调方法
    [_apiGetActivityInfo setFinishBlock:^(APIHelper *apiHelper, NSError *error) {
        // 是否已经显示活动
        BOOL isShowedBaner = NO;
        // 正常返回
        if (apiHelper.data.length > 0) {
            BaseModel *mBase = [[BaseModel alloc] initWithData:apiHelper.data];
            if (mBase) {
                if (mBase.returncode == 0) {
                    vCarList.mActivity = [[UCActivityModel alloc] initWithJson:mBase.result];
                    if (vCarList.mActivity.adlist.count > 0) {
                        [vCarList showBaner:((AdlistItemModel *)vCarList.mActivity.adlist[0]).title];
                        isShowedBaner = YES;
                    }
                }
            }
        ...
        if (!isShowedBaner)
            vCarList.tvCarList.tableHeaderView = nil;
    }];
    // 获取活动专区信息
    [_apiGetActivityInfo getActivityInfo:[AMCacheManage currentArea].pid cid:[AMCacheManage currentArea].cid];
}
